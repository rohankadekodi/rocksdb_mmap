...
// Current the following isn't equivalent to OS_LINUX.
    ...
    while (std::getline(benchmark_stream, name, ',')) {
      ...
      if (method != nullptr) {
        ...
        for (int i = 0; i < num_warmup; i++) {
          RunBenchmark(1, 1, num_threads, name, method);
        }
 ...
 private:
  ...
  Stats RunBenchmark(int benchmark_id, int num_benchmarks, int n, Slice name,
                     void (Benchmark::*method)(ThreadState*)) {
    SharedState shared;
    shared.total = n;
    ...
    for (int i = 0; i < n; i++) {
      ...
      arg[i].method = method;
      arg[i].shared = &shared;
      arg[i].thread = new ThreadState((benchmark_id*n) + i);
