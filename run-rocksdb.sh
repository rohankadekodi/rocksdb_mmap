...
iotop -a -btoq -d 0.1 > /tmp/rocksdb-iotop-100M &

./db_bench --benchmarks=fillrandom --num=$r --threads=$t --value_size=$vs --cache_size=$cs --bloom_bits=10 --open_files=$of --db=/mnt/raid0/flsm/rocksdb-new-100M --compression_ratio=0.5 --compression_type=none --write_buffer_size=$wbs --use_existing_db=0
